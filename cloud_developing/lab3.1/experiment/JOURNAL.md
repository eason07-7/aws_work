...
### [2026-09-14 14:58] 事件 #2 — 首刷 Task2–5 全 boto3 自動化（bucket + PAB + IP policy + 80 檔上傳）

**當下模型**：opus-5
**現象**：Lab 3.1 所有任務皆 CLI/SDK 操作，無 Console-only 動作
**原因**：自動化優先：用 boto3 重現 create-bucket / put_public_access_block / put_bucket_policy / 遞迴 upload_file(CacheControl=max-age=0)
**處理**：寫 lab3_1_steps.py；code.zip 下載到 作業要求/cloudsever/Lab 3.1/code/；bucket=el-2026-09-14-s3site；policy 白名單 IP 用 api.ipify.org 自動抓
**驗證**：PAB={BlockPublicAcls,IgnorePublicAcls,RestrictPublicBuckets}=True/BlockPublicPolicy=False；policy 2 statements；本機無簽名 GET index.html HTTP 200；head_object CacheControl=max-age=0
**Tags**：#s3 #bucket-policy #public-access-block #static-website #first-pass

---

### [2026-09-14 15:09] 事件 #3 — Submit → 100/100；提煉 SUCCESS.md

**當下模型**：opus-5
**現象**：grader 滿分
**原因**：bucket/PAB/policy/objects 皆 boto3 可完成
**處理**：寫 SUCCESS.md
**驗證**：Grades 100/100
