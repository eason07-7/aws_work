...
### [2026-09-14 15:20] 事件 #2 — 首刷 Task2–7 全 boto3 自動化（FoodProducts + 26 items + special_GSI）

**當下模型**：opus-5
**現象**：所有 Task 皆 CLI/SDK；照作業順序重現含兩處刻意失敗（ConditionalCheckFailed、BatchWriteItem duplicates ValidationException）
**原因**：自動化優先；Console 的 Scan/Delete items 用 ddb.scan / batch_writer.delete_item 等價
**處理**：寫 lab5_1_steps.py；code.zip 解壓在 Lab 5.1/code/；GSI 建立後輪詢 describe_table 到 ACTIVE
**驗證**：table ACTIVE、scan 26 items、get_item chocolate cake 4095、special_GSI ACTIVE、filter scan 6 specials；表在執行前已存在（idempotent 沿用）
**Tags**：#dynamodb #put-item #condition-expression #batch-writer #gsi #first-pass

---

### [2026-09-14 15:47] 事件 #3 — Submit → 100/100；提煉 SUCCESS.md

**當下模型**：opus-5
**現象**：grader 滿分
**原因**：最終狀態檢查全由 boto3 達成
**處理**：寫 SUCCESS.md，同步公開倉 cloud_developing/lab5.1
**驗證**：Grades 100/100
