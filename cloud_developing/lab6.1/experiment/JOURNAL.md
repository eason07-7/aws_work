...
### [2026-09-14 16:03] 事件 #2 — 首刷 Task1–6 全 boto3 自動化（ProductsApi 3 mock endpoints + prod stage + config.js）

**當下模型**：opus-5
**現象**：所有 Task 皆 SDK 可做；Console 的 TEST 用 test_invoke_method 等價；Deploy API 用 create_deployment(stageName=prod)
**原因**：自動化優先
**處理**：寫 lab6_1_steps.py；setup.sh 等價（上傳 80 檔 + IP policy 到預建 *s3bucket*）；三個 MOCK 方法照原腳本參數；config.js 寫 Invoke URL 後 upload_file(ContentType=application/js)
**驗證**：test_invoke_method 三個皆 200；真實 HTTP GET /products 3 筆、/products/on_offer 1 筆、POST /create_report 回 msg_str；api_id=jzlokhg7c9
**Tags**：#api-gateway #mock-integration #cors #deploy-stage #first-pass

---

### [2026-09-14 16:07] 事件 #3 — Submit → 100/100；提煉 SUCCESS.md

**當下模型**：opus-5
**現象**：grader 滿分
**原因**：API/stage/config.js 最終狀態全由 boto3 達成
**處理**：寫 SUCCESS.md，同步公開倉 cloud_developing/lab6.1
**驗證**：Grades 100/100
