...
### [2026-09-14 17:56] 事件 #2 — 首刷 Task1 手動(IDE setup.sh + 公鑰) + Task2–8 全自動（SG/subnet group/Memcached/pymemcache 腳本/EB MEMC_HOST）

**當下模型**：opus-5
**現象**：預建 Aurora、EB、ECR；setup.sh 在 IDE seed+push+EB 換版；Memcached 叢集與 SG 可先用 boto3 開建
**原因**：自動化優先；Console 步驟對應 authorize_security_group_ingress、create_cache_subnet_group、create_cache_cluster、eb.update_environment(OptionSettings MEMC_HOST)
**處理**：lab9_1_steps.py 分段 idempotent；SSH 用 <學號>/lab_ide_key；踩坑：(1) 參數群組 API 名 default.memcached1.6（Console 顯示 default.memcached.1.6）；(2) update_item.py 實際印 'Cache purged.' 非文件的 'Purged the cache'，斷言改小寫比對 purged
**驗證**：find_all 第 1 次 DB 第 2 次 cache；update/create/delete 後 purge；EB /beans 先 results from Db 再 results from Cache
**Tags**：#elasticache #memcached #pymemcache #write-through #lazy-loading #elastic-beanstalk #manual-required #first-pass

---

### [2026-09-14 17:58] 事件 #3 — Submit → 100/100；提煉 SUCCESS.md

**當下模型**：opus-5
**現象**：grader 滿分
**原因**：SG/subnet group/cache/beans 資料狀態/EB 環境變數皆達成
**處理**：寫 SUCCESS.md，同步公開倉 cloud_developing/lab9.1
**驗證**：Grades 100/100
